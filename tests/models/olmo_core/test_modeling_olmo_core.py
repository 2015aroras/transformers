...
"""Testing suite for the PyTorch OLMo Core model."""
...
from transformers import OlmoCoreConfig, is_torch_available, set_seed
...
class OlmoCoreModelTester:
    ...
    def create_and_check_model(
        ...
        model = OlmoCoreModel(config=config)
...
@require_torch
class OlmoCoreModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (OlmoCoreModel, OlmoCoreForCausalLM) if is_torch_available() else ()
    test_pruning = False
    fx_compatible = False
    ...
    def setUp(self):
        self.model_tester = OlmoCoreModelTester(self)
        self.config_tester = ConfigTester(self, config_class=OlmoCoreConfig, hidden_size=37)
    ...
    @unittest.skip(reason="OLMo Core does not support head pruning.")
    ...
    def test_model_rope_scaling(self, scaling_type):
        ...
        original_model = OlmoCoreModel(config)
        ...
        scaled_model = OlmoCoreModel(config)
...
@require_torch
class OlmoCoreIntegrationTest(unittest.TestCase):
    @slow
    def test_model_1b_logits_bfloat16(self):
        input_ids = [[1, 306, 4658, 278, 6593, 310, 2834, 338]]
        model = OlmoCoreForCausalLM.from_pretrained("").to(torch.bfloat16)
    ...
    def test_model_7b_logits(self):
        input_ids = [[1, 306, 4658, 278, 6593, 310, 2834, 338]]
        model = OlmoCoreForCausalLM.from_pretrained("shanearora/OLMo Core-7B-1124-hf", device_map="auto")
        out = model(torch.tensor(input_ids)).logits.float()
        # Expected mean on dim = -1
    ...
    def test_model_7b_greedy_generation(self):
        ...
        tokenizer = AutoTokenizer.from_pretrained("shanearora/OLMo Core-7B-1124-hf", device_map="auto")
        model = OlmoCoreForCausalLM.from_pretrained("shanearora/OLMo Core-7B-1124-hf", device_map="auto")
    ...
    def test_simple_encode_decode(self):
        rust_tokenizer = AutoTokenizer.from_pretrained("shanearora/OLMo Core-7B-1124-hf")
    ...
    def test_export_static_cache(self):
        ...
        olmo_core_model = "shanearora/OLMo Core-7B-1124-hf"

        tokenizer = AutoTokenizer.from_pretrained(olmo_core_model, pad_token="</s>", padding_side="right")
        ...
        model = OlmoCoreForCausalLM.from_pretrained(
            olmo_core_model,
            device_map=device,
            torch_dtype=dtype,
