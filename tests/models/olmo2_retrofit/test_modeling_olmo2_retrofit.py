...
"""Testing suite for the PyTorch OLMo2 Retrofit model."""
...
from transformers import Olmo2RetrofitConfig, is_torch_available, set_seed
...
class Olmo2RetrofitModelTester:
    def __init__(
        self,
        parent,
        batch_size=13,
    ...
    def create_and_check_model(
        ...
        model = Olmo2RetrofitModel(config=config)
...
@require_torch
class Olmo2RetrofitModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (Olmo2RetrofitModel, Olmo2RetrofitForCausalLM) if is_torch_available() else ()
    test_pruning = False
    fx_compatible = False
    ...
    def setUp(self):
        self.model_tester = Olmo2RetrofitModelTester(self)
        self.config_tester = ConfigTester(self, config_class=Olmo2RetrofitConfig, hidden_size=37)
    ...
    @unittest.skip(reason="OLMo2 Retrofit does not support head pruning.")
    ...
    def test_model_rope_scaling(self, scaling_type):
        ...
        original_model = Olmo2RetrofitModel(config)
        ...
        scaled_model = Olmo2RetrofitModel(config)
...
@require_torch
class Olmo2RetrofitIntegrationTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_model_1b_logits_bfloat16(self):
        ...
        model = Olmo2RetrofitForCausalLM.from_pretrained("allenai/OLMo-2-0425-1B").to(torch_device, torch.bfloat16)
    ...
    def test_model_7b_logits(self):
        input_ids = [[1, 306, 4658, 278, 6593, 310, 2834, 338]]
        model = Olmo2RetrofitForCausalLM.from_pretrained("shanearora/OLMo2 Retrofit-7B-1124-hf").to(torch_device, dtype=torch.bfloat16)
        out = model(torch.tensor(input_ids, device=torch_device)).logits.float()
        # Expected mean on dim = -1
        expectations = Expectations(
            {
    ...
    def test_model_7b_greedy_generation(self):
        ...
        tokenizer = AutoTokenizer.from_pretrained("shanearora/OLMo2 Retrofit-7B-1124-hf", device_map="auto")
        model = Olmo2RetrofitForCausalLM.from_pretrained("shanearora/OLMo2 Retrofit-7B-1124-hf", device_map="auto")
    ...
    def test_simple_encode_decode(self):
        rust_tokenizer = AutoTokenizer.from_pretrained("shanearora/OLMo2 Retrofit-7B-1124-hf")
    ...
    def test_export_static_cache(self):
        ...
        olmo2_retrofit_model = "shanearora/OLMo2 Retrofit-7B-1124-hf"

        tokenizer = AutoTokenizer.from_pretrained(olmo2_retrofit_model, pad_token="</s>", padding_side="right")
        ...
        model = Olmo2RetrofitForCausalLM.from_pretrained(
            olmo2_retrofit_model,
            device_map=device,
            torch_dtype=dtype,
