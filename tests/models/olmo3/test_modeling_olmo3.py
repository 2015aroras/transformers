...
class Olmo3ModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    @unittest.skip(reason="OLMo3 does not support head pruning.")
...
class Olmo3IntegrationTest(unittest.TestCase):
    ...
    def test_model_7b_logits(self):
        input_ids = [[1, 306, 4658, 278, 6593, 310, 2834, 338]]
        model = Olmo3ForCausalLM.from_pretrained("shanearora/OLMo3-7B-1124-hf").to(torch_device, dtype=torch.bfloat16)
        out = model(torch.tensor(input_ids, device=torch_device)).logits.float()
        # Expected mean on dim = -1
        expectations = Expectations(
            {
    ...
    def test_model_7b_greedy_generation(self):
        ...
        tokenizer = AutoTokenizer.from_pretrained("shanearora/OLMo3-7B-1124-hf", device_map="auto")
        model = Olmo3ForCausalLM.from_pretrained("shanearora/OLMo3-7B-1124-hf", device_map="auto")
    ...
    def test_simple_encode_decode(self):
        rust_tokenizer = AutoTokenizer.from_pretrained("shanearora/OLMo3-7B-1124-hf")
    ...
    def test_export_static_cache(self):
        ...
        olmo3_model = "shanearora/OLMo3-7B-1124-hf"
