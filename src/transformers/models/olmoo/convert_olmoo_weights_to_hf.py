...
```
python src/transformers/models/olmoo/convert_olmoo_weights_to_hf.py \
    --input_dir /path/to/downloaded/olmoo/weights --model_size 7B --output_dir /output/path
```

Thereafter, models can be loaded via:

```py
from transformers import OlmooForCausalLM, AutoTokenizer

model = OlmooForCausalLM.from_pretrained("/output/path")
...
def write_model(model_path, input_base_path, tokenizer_path=None, safe_serialization=True, fix_eos_token_id=True):
    ...
    max_position_embeddings = olmoo_config["max_sequence_length"]

    vocab_size = olmoo_config.get("embedding_size", olmoo_config["vocab_size"])

    if olmoo_config.get("n_kv_heads", None) is not None:
        num_key_value_heads = olmoo_config["n_kv_heads"]  # for GQA / MQA
    elif olmoo_config["multi_query_attention"]:  # compatibility with other checkpoints
    ...
    if olmoo_config.get("mlp_hidden_size", None) is not None:
        intermediate_size = olmoo_config["mlp_hidden_size"] // 2
    else:
        intermediate_size = (dim * olmoo_config["mlp_ratio"]) // 2
    ...
    config = OlmooConfig(
        ...
        pad_token_id=olmoo_config["pad_token_id"],
        bos_token_id=None,
        eos_token_id=olmoo_config["eos_token_id"],
        tie_word_embeddings=olmoo_config["weight_tying"],
        rope_theta=base,
    ...
    print("Loading the checkpoint in a OLMoo model.")
    model = OlmooForCausalLM.from_pretrained(tmp_model_path, torch_dtype=torch.float32, low_cpu_mem_usage=True)
...
def _write_tokenizer(
    output_path: Path, config: OlmooConfig, input_tokenizer_path: Path, fix_eos_token_id: bool = True
) -> None:
    ...
    if fix_eos_token_id and eos_token_id == 0:
        # Fixing a bug in OLMoo where eos token id was incorrectly set
...
def main():
    ...
    parser.add_argument(
        "--input_dir",
        required=True,
        help="Location of OLMoo weights, which contains config.yaml and model.pt.",
    )
    parser.add_argument(
        "--tokenizer_json_path",
        default=None,
        help="Location of OLMoo tokenizer json file.",
    )
    parser.add_argument(
        "--output_dir",
        required=True,
    ...
    # Different OLMoo versions used different default values for max_position_embeddings, hence the need to be able to specify which version is being used.
