...
def main(olmoo_directory: Path, hf_directory: Path, hf_revision: Optional[str]):
    ...
    hf_model = AutoModelForCausalLM.from_pretrained(
        str(hf_directory), revision=hf_revision, device_map="auto", trust_remote_code=True
    ).to(device="cpu")
    ...
    olmoo_model = olmo.OLMo(model_config)

    # olmoo_out = olmoo_model(in_vec)

    # print("Before loading OLMoo state, logits diff norm", torch.norm(hf_out.logits - olmoo_out.logits))
    # print("Before loading OLMoo state, logits diff max", torch.max(hf_out.logits - olmoo_out.logits))

    print("Loading OLMoo state")
    model_state = torch.load(str(olmoo_directory / "model.pt"), map_location="cpu")
