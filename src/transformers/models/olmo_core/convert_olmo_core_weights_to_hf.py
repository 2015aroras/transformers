...
from transformers import OlmoCoreConfig, OlmoCoreForCausalLM
...
```
python src/transformers/models/olmo_core/convert_olmo_core_weights_to_hf.py \
    --input_dir /path/to/downloaded/olmo_core/weights --model_size 7B --output_dir /output/path
```

Thereafter, models can be loaded via:

```py
from transformers import OlmoCoreForCausalLM, AutoTokenizer

model = OlmoCoreForCausalLM.from_pretrained("/output/path")
...
def write_model(
    ...
    olmo_core_config = yaml.safe_load(config_path.read_text())["model"]

    if not olmo_core_config.get("attention_layer_norm", False):
        raise RuntimeError("OLMo Core checkpoints must have attention layer norm")
    if not olmo_core_config.get("norm_after", False):
        raise RuntimeError("OLMo Core checkpoints must set norm_after to True")

    n_layers = olmo_core_config["n_layers"]
    n_heads = olmo_core_config["n_heads"]
    dim = olmo_core_config["d_model"]
    dims_per_head = dim // n_heads
    base = olmo_core_config["rope_theta"]
    inv_freq = 1.0 / (base ** (torch.arange(0, dims_per_head, 2).float() / dims_per_head))
    max_position_embeddings = olmo_core_config["max_sequence_length"]

    vocab_size = olmo_core_config.get("embedding_size", olmo_core_config["vocab_size"])

    if olmo_core_config.get("n_kv_heads", None) is not None:
        num_key_value_heads = olmo_core_config["n_kv_heads"]  # for GQA / MQA
    elif olmo_core_config["multi_query_attention"]:  # compatibility with other checkpoints
    ...
    if olmo_core_config.get("mlp_hidden_size", None) is not None:
        intermediate_size = olmo_core_config["mlp_hidden_size"] // 2
    else:
        intermediate_size = (dim * olmo_core_config["mlp_ratio"]) // 2

    if fix_eos_token_id and olmo_core_config["eos_token_id"] == 0:
        # Fixing a bug in OLMo where eos token id was incorrectly set
        print("Changing eos_token_id from 0 to 50279.")
        olmo_core_config["eos_token_id"] = 50279

    config = OlmoCoreConfig(
        ...
        pad_token_id=olmo_core_config["pad_token_id"],
        bos_token_id=None,
        eos_token_id=olmo_core_config["eos_token_id"],
        tie_word_embeddings=olmo_core_config["weight_tying"],
        rms_norm_eps=olmo_core_config["layer_norm_eps"],
        rope_theta=base,
    )
    ...
    print("Loading the checkpoint in a OLMo Core model.")
    model = OlmoCoreForCausalLM.from_pretrained(tmp_model_path, torch_dtype=torch.float32, low_cpu_mem_usage=True)
...
def main():
    ...
    parser.add_argument(
        "--input_dir",
        required=True,
        help="Location of OLMo Core weights, which contains config.yaml and model.pt.",
    )
    parser.add_argument(
        "--no_tokenizer",
        action="store_false",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Location of OLMo Core tokenizer json file. Defaults to what is set in the config file.",
    )
    parser.add_argument(
        "--output_dir",
        required=True,
