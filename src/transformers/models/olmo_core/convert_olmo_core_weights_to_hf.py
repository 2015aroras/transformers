...
def convert_checkpoint_to_hf(
    ...
    *,
    ...
    if validate:
        ...
        validate_conversion(output_path, model, tokenizer_config.vocab_size, debug=debug, device=device)
    ...
    if debug:
        ...
        for olmo_core_state_name, (_, olmo_core_tensor) in sorted(
            ...
            if olmo_core_tensor.shape != hf_tensor.shape:
                log.info(f"{olmo_core_state_name} shape mismatch: {olmo_core_tensor.shape} {hf_tensor.shape}")
            else:
                log.info(f"{olmo_core_state_name} norm diff: {torch.norm(olmo_core_tensor - hf_tensor)}")
...
def load_config(checkpoint_input_dir: PathOrStr) -> Optional[dict]:
    ...
    if "model" not in config_dict:
        raise RuntimeError(f"Config file at {checkpoint_input_dir} is not an OLMo core experiment config, ignoring")
