...
def main(olmo_directory: Path, hf_directory: Path, hf_revision: Optional[str]):
    ...
    hf_model = AutoModelForCausalLM.from_pretrained(str(hf_directory), revision=hf_revision, device_map="auto", trust_remote_code=True).to(device="cpu")
    ...
    # print("Before loading OLMo state, logits diff norm", torch.norm(hf_out.logits - olmo_out.logits))
    # print("Before loading OLMo state, logits diff max", torch.max(hf_out.logits - olmo_out.logits))
