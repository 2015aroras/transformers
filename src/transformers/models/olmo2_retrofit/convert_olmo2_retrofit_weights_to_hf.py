...
from transformers import Olmo2RetrofitConfig, Olmo2RetrofitForCausalLM
...
```
python src/transformers/models/olmo2_retrofit/convert_olmo2_retrofit_weights_to_hf.py \
    --input_dir /path/to/downloaded/olmo2_retrofit/weights --model_size 7B --output_dir /output/path
```

Thereafter, models can be loaded via:

```py
from transformers import Olmo2RetrofitForCausalLM, AutoTokenizer

model = Olmo2RetrofitForCausalLM.from_pretrained("/output/path")
...
def write_model(
    ...
    olmo2_retrofit_config = yaml.safe_load(config_path.read_text())["model"]

    if not olmo2_retrofit_config.get("attention_layer_norm", False):
        raise RuntimeError("OLMo2 Retrofit checkpoints must have attention layer norm")
    if not olmo2_retrofit_config.get("norm_after", False):
        raise RuntimeError("OLMo2 Retrofit checkpoints must set norm_after to True")

    n_layers = olmo2_retrofit_config["n_layers"]
    n_heads = olmo2_retrofit_config["n_heads"]
    dim = olmo2_retrofit_config["d_model"]
    dims_per_head = dim // n_heads
    base = olmo2_retrofit_config["rope_theta"]
    inv_freq = 1.0 / (base ** (torch.arange(0, dims_per_head, 2).float() / dims_per_head))
    max_position_embeddings = olmo2_retrofit_config["max_sequence_length"]

    vocab_size = olmo2_retrofit_config.get("embedding_size", olmo2_retrofit_config["vocab_size"])

    if olmo2_retrofit_config.get("n_kv_heads", None) is not None:
        num_key_value_heads = olmo2_retrofit_config["n_kv_heads"]  # for GQA / MQA
    elif olmo2_retrofit_config["multi_query_attention"]:  # compatibility with other checkpoints
    ...
    if olmo2_retrofit_config.get("mlp_hidden_size", None) is not None:
        intermediate_size = olmo2_retrofit_config["mlp_hidden_size"] // 2
    else:
        intermediate_size = (dim * olmo2_retrofit_config["mlp_ratio"]) // 2

    if fix_eos_token_id and olmo2_retrofit_config["eos_token_id"] == 0:
        # Fixing a bug in OLMo where eos token id was incorrectly set
        print("Changing eos_token_id from 0 to 50279.")
        olmo2_retrofit_config["eos_token_id"] = 50279

    config = Olmo2RetrofitConfig(
        ...
        pad_token_id=olmo2_retrofit_config["pad_token_id"],
        bos_token_id=None,
        eos_token_id=olmo2_retrofit_config["eos_token_id"],
        tie_word_embeddings=olmo2_retrofit_config["weight_tying"],
        rms_norm_eps=olmo2_retrofit_config["layer_norm_eps"],
        rope_theta=base,
    )
    ...
    print("Loading the checkpoint in a OLMo2 Retrofit model.")
    model = Olmo2RetrofitForCausalLM.from_pretrained(tmp_model_path, torch_dtype=torch.float32)
...
def main():
    ...
    parser.add_argument(
        "--input_dir",
        required=True,
        help="Location of OLMo2 Retrofit weights, which contains config.yaml and model.pt.",
    )
    parser.add_argument(
        "--no_tokenizer",
        action="store_false",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Location of OLMo2 Retrofit tokenizer json file. Defaults to what is set in the config file.",
    )
    parser.add_argument(
        "--output_dir",
        required=True,
