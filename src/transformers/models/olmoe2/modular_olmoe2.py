...
class Olmoe2Config(Olmo2Config):
    r"""
...
class Olmoe2Attention(Olmo2Attention):
    def __init__(self, config: Olmoe2Config, layer_idx: int):
        ...
        self.q_norm = Olmoe2RMSNorm(self.head_dim, config.rms_norm_eps)
        self.k_norm = Olmoe2RMSNorm(self.head_dim, config.rms_norm_eps)
...
class Olmoe2DecoderLayer(Olmo2DecoderLayer):
    pass
...
class Olmoe2PreTrainedModel(Olmo2PreTrainedModel):
    pass


# The OLMo 3 model is identical to the OLMo 2 model, except:
# - Sliding window attention is used for 3 out of 4 layers.
class Olmoe2Model(Olmo2Model):
...
class Olmoe2ForCausalLM(Olmo2ForCausalLM):
    pass
